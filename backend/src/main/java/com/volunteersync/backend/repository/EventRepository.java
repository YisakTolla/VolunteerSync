...
public interface EventRepository extends JpaRepository<Event, Long> {
       ...
       List<Event> findByEventTypesAndStatus(@Param("eventTypes") List<EventType> eventTypes,
       ...
       List<Event> findEventsAccessibleForSkillLevel(@Param("skillLevel") SkillLevel skillLevel,
                     @Param("status") EventStatus status);
       ...
       @Query("SELECT e FROM Event e WHERE e.status = :status AND " +
                     ...
                     "ORDER BY e.startDate ASC")
       List<Event> findEventsByLocationIncludingVirtual(@Param("location") String location,
                     @Param("status") EventStatus status);
       ...
       @Query("SELECT e FROM Event e WHERE e.status = :status AND " +
                     ...
                     "(e.isWeekendsOnly = true OR " +
                     "CAST(DAYOFWEEK(e.startDate) AS int) IN (1, 7)) " +
                     "ORDER BY e.startDate ASC")
       ...
       List<Event> findEventsThisMonth(@Param("year") int year, @Param("month") int month,
                     @Param("status") EventStatus status);
       ...
       List<Event> findUrgentEvents(@Param("now") LocalDateTime now,
                     @Param("twentyFourHours") LocalDateTime twentyFourHours);
       ...
       List<Event> findEventsStartingSoon(@Param("now") LocalDateTime now,
                     @Param("seventyTwoHours") LocalDateTime seventyTwoHours);
